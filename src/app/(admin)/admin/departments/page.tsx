...
import { LayoutDashboard, Plus, Building2, TrendingUp, AlertTriangle, CheckCircle2, DollarSign, Users, Network } from "lucide-react";
import OrgChartTree from "@/components/admin/OrgChartTree";

export default function DepartmentsAdmin() {
    const [activeTab, setActiveTab] = useState<'departments' | 'org_chart'>('departments');
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Departmanlar & Organizasyon</h1>
                    <p className="text-slate-500">Şirket yapısını, bütçeleri ve hiyerarşiyi yönetin.</p>
                </div>
                ...
                <div className="flex gap-2">
                    <div className="flex bg-slate-100 p-1 rounded-lg">
                        <button
                            ...
                            className={`px-3 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'departments' ? 'bg-white shadow text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                        ...
                        <button
                            ...
                            className={`px-3 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'org_chart' ? 'bg-white shadow text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                    ...
                    {activeTab === 'departments' && (
                        ...
                        >
                            ...
                            <span className="hidden sm:inline">Yeni Departman</span>
                        </button>
                    )}
                </div>
            </div>
